...
def getBonuses(performance):
  bonus = []
  for e in range(len(performance)):
    current_perf = 1
    if e-1>=0 and performance[e-1] < performance[e]: 
      current_perf += 1
    if e+1 < len(performance) and performance[e+1] < performance[e]:
      current_perf += 1
    bonus.append(current_perf)
  return bonus
